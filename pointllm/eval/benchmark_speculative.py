...
from pointllm.eval.speculative_decode import speculative_decode
...
def run_vanilla(model, tokenizer, input_ids, point_clouds, max_new_tokens, do_sample, temperature, top_k, top_p):
...
def run_speculative(args, model, assistant_model, tokenizer, input_ids, point_clouds, max_new_tokens):
    total_max_length = int(input_ids.shape[1] + max_new_tokens)
    torch.cuda.synchronize()
    start = time.perf_counter()
    with torch.inference_mode():
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Maximum number of newly generated tokens for both vanilla and speculative decoding.",
    )
    parser.add_argument("--torch_dtype", type=str, default="bfloat16", choices=["float32", "float16", "bfloat16"])
    parser.add_argument("--speculative_mode", type=str, default="linear", choices=["linear"])
    parser.add_argument("--num_assistant_tokens", type=int, default=8)
    parser.add_argument("--do_sample", action="store_true", help="Enable sampling. Default is greedy decoding for stable speed benchmarks.")
    ...
    if not args.do_sample and (args.top_k > 0 or args.top_p < 1.0):
        print("[INFO] Greedy mode is enabled; top_k/top_p are ignored.")
    if args.do_sample and args.speculative_mode == "linear":
        print("[INFO] Sampling mode may reduce draft acceptance and speedup due to target/draft mismatch.")
    ...
    for object_id in object_ids:
        ...
        spec_output, spec_time, spec_tokens, stats = run_speculative(
            args, model, assistant_model, tokenizer, input_ids, point_clouds, max_new_tokens=args.max_new_tokens
        )
