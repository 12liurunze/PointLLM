...
class DataArguments:
    ...
    skip_missing_files: bool = field(default=True)
...
def main():
    ...
    use_amp = device.type == "cuda" and (distill_args.bf16 or distill_args.fp16)
    amp_dtype = torch.bfloat16 if distill_args.bf16 else torch.float16
    ...
    while global_step < total_steps:
        for batch in train_loader:
            ...
            if point_clouds is not None:
                # Keep point clouds in FP32 for PointBERT stability. AMP/autocast will downcast
                # compatible ops when bf16/fp16 is enabled.
                point_clouds = point_clouds.to(device=device, dtype=torch.float32)
