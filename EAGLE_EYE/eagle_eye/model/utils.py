...
def generate_tree_buffers(tree_choices, device="cuda"):
    ...
    for row in retrieve_indices:
        valid = row[row > 0]
        if valid.numel() == 0:
            retrieve_paths.append([])
        else:
            retrieve_paths.append(sorted_tree_choices[int(valid[-1].item()) - 1])
    ...
    tree_buffers["retrieve_paths"] = retrieve_paths
    return tree_buffers
...
def update_inference_inputs(
    ...
    topk_input_ids = torch.cat((input_ids, token.to(input_ids.device)), dim=1)
    if hasattr(model, "draft_input_ids") and model.draft_input_ids is not None:
        accepted_tokens = candidates[None, best_candidate, : accept_length + 1].to(model.draft_input_ids.device)
        model.draft_input_ids = torch.cat((model.draft_input_ids, accepted_tokens), dim=1)
        draft_topk_input_ids = torch.cat((model.draft_input_ids, token.to(model.draft_input_ids.device)), dim=1)
        topk_input_ids = draft_topk_input_ids
