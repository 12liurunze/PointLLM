...
def run_eagle(args, baseline_results, helpers):
    ...
    for sample in baseline_results:
        try:
            ...
            torch.cuda.synchronize()
            start_time = time.perf_counter()
            output_ids, eagle_stats = model.eagenerate(
                input_ids=input_ids,
                point_clouds=point_clouds,
                stopping_criteria=[stopping_criteria],
            ...
            result = {
                **sample,
                ...
                "eagle_accepted_tokens_total": eagle_stats["accepted_tokens_total"],
                "eagle_accept_steps": eagle_stats["accept_steps"],
                "eagle_mat": eagle_stats["mat"],
            }
...
def summarize(results, skipped):
    ...
    total_accepted_tokens = sum(item.get("eagle_accepted_tokens_total", 0) for item in results)
    total_accept_steps = sum(item.get("eagle_accept_steps", 0) for item in results)
    mat = total_accepted_tokens / total_accept_steps if total_accept_steps > 0 else None
    return {
        ...
        "eagle_accepted_tokens_total": total_accepted_tokens,
        "eagle_accept_steps": total_accept_steps,
        "mat": mat,
    }
...
def main():
    ...
    parser.add_argument("--tree-choices", default=None, help="Python literal list or name from eagle_eye.model.choices; defaults to mc_sim_7b_63.")
    parser.add_argument("--device-map", default=None)
    args = parser.parse_args()
    from eagle_eye.model import choices as tree_choice_defs
    if args.tree_choices:
        if hasattr(tree_choice_defs, args.tree_choices):
            args.tree_choices = getattr(tree_choice_defs, args.tree_choices)
        else:
            args.tree_choices = ast.literal_eval(args.tree_choices)
    else:
        args.tree_choices = tree_choice_defs.mc_sim_7b_63
